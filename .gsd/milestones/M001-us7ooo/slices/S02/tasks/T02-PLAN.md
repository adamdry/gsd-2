...
2. Add test sections — test each regex pattern directly (since most are module-scoped, extract the pattern into a local const and test it):
   ...
   - (c) `SLICE_BRANCH_RE` (import from `../worktree.ts`) — matches `gsd/M001-us7ooo/S01`, `gsd/M-abc123-001/S01`, `gsd/worktree/M001-us7ooo/S01`, `gsd/worktree/M-abc123-001/S01`, rejects `gsd/S01`, `main`
   - (d) Milestone detection regex `/^M(?:-[a-z0-9]{6}-)?\d+/` — matches both formats, rejects `S01`
   - (e) `MILESTONE_CONTEXT_RE` pattern `/M(?:-[a-z0-9]{6}-)?\d+-CONTEXT\.md$/` — matches `M001-us7ooo-CONTEXT.md`, `M-abc123-001-CONTEXT.md`, rejects `M001-us7ooo-ROADMAP.md`
