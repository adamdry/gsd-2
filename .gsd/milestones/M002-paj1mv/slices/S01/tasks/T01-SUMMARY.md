...
patterns_established:
  - New ID format M{seq}-{rand6} e.g. M001-d24bw5-abc123 (seq in group 1, prefix in group 2)
...
Migrated all test data across 3 test files: `M-abc123-001` → `M001-d24bw5-abc123` style across ~114 references. Updated regex match/reject cases, parse expectations (group swap), sort arrays, and assertion patterns.
...
- `MILESTONE_ID_RE.test('M001-d24bw5-abc123')` → true, validates new format ✅
- `extractMilestoneSeq('M001-d24bw5-abc123')` → `'001'` ✅
- `parseMilestoneId('M001-d24bw5-abc123')` → `{ prefix: 'abc123', num: 1 }` ✅
