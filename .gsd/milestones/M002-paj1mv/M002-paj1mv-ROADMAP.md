...
**Vision:** Swap the unreleased unique milestone ID format from `M-{rand6}-{seq}` to `M{seq}-{rand6}` (e.g. `M001-d24bw5-abc123`) with zero traces of the old format remaining. Clean replacement — no migration, no backwards compatibility.

## Success Criteria

- `nextMilestoneId([], true)` generates `M001-d24bw5-{rand6}` format IDs
...
- `nextMilestoneId(milestoneIds, true)` generates `M{seq}-{rand6}` format (e.g. `M001-d24bw5-abc123`)
- `MILESTONE_ID_RE`, `extractMilestoneSeq`, `parseMilestoneId` all handle `M001-d24bw5-abc123` format
...
- [x] **S01: Format swap — production code, tests, and docs** `risk:low` `depends:[]`
  > After this: `nextMilestoneId([], true)` returns `M001-d24bw5-{rand6}`, all ~208 test assertions pass with new format, grep confirms zero traces of old `M-{rand6}-{seq}` pattern anywhere in the codebase.
...
- 3 test files → all test data using `M001-d24bw5-abc123` format instead of `M-abc123-001`
