...
2. Add test sections — test each regex pattern directly (since most are module-scoped, extract the pattern into a local const and test it):
   - (a) Directory scanning regex `/^(M(?:-[a-z0-9]{6}-)?\d+)/` — matches `M001-d24bw5`, `M-abc123-001`, rejects `S01`, `X001`
   - (b) Title-strip regex `/^M(?:-[a-z0-9]{6}-)?\d+[^:]*:\s*/` — strips `M001-d24bw5: Title` → `Title`, strips `M-abc123-001: Title` → `Title`, strips `M001-d24bw5 — Unique Milestone IDs: Foo` preserving existing behavior
   - (c) `SLICE_BRANCH_RE` (import from `../worktree.ts`) — matches `gsd/M001-d24bw5/S01`, `gsd/M-abc123-001/S01`, `gsd/worktree/M001-d24bw5/S01`, `gsd/worktree/M-abc123-001/S01`, rejects `gsd/S01`, `main`
   - (d) Milestone detection regex `/^M(?:-[a-z0-9]{6}-)?\d+/` — matches both formats, rejects `S01`
   - (e) `MILESTONE_CONTEXT_RE` pattern `/M(?:-[a-z0-9]{6}-)?\d+-CONTEXT\.md$/` — matches `M001-d24bw5-CONTEXT.md`, `M-abc123-001-CONTEXT.md`, rejects `M001-d24bw5-ROADMAP.md`
   - (f) Prompt dispatch regexes — test capture of milestone ID from both old and new format execute/resume prompt strings
   - (g) `milestoneIdSort` with mixed arrays — `['M-abc123-002', 'M001-d24bw5', 'M-xyz789-001']` sorts to sequential order
...
- [ ] Every section tests both old (`M001-d24bw5`) and new (`M-abc123-001`) format inputs
