...
**Goal:** All code paths accept both `M001-d24bw5` and `M-abc123-001` milestone ID formats — directory scanning, title stripping, branch parsing, prompt dispatch, context write-gating, and dispatch guarding work with mixed-format milestone directories.
...
- [x] **T02: Write regex-hardening test suite proving all sites accept both formats** `est:20m`
  - Why: Each widened regex and the dispatch-guard refactor needs targeted proof that both `M001-d24bw5` and `M-abc123-001` work — without tests, regressions are silent
