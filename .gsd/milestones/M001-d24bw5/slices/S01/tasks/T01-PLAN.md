...
2. Add exported constants and functions above the existing `maxMilestoneNum`:
   - `MILESTONE_ID_RE`: `/^M(?:-[a-z0-9]{6}-)?\d{3}$/` — matches both `M001-d24bw5` and `M-abc123-001`
...
5. Widen `findMilestoneIds()` regex from `/^(M\d+)/` to a pattern that captures both `M001-d24bw5` and `M-abc123-001` from directory names like `M001-d24bw5-CONTEXT` or `M-abc123-001/`
