...
**Goal:** `nextMilestoneId()` returns `M-{rand6}-{seq}` when `unique_milestone_ids: true`, `M001-d24bw5`-style when false/absent. Preferences system supports the new boolean field end-to-end.
**Demo:** Unit tests prove generation, parsing, config plumbing, and call site wiring all work correctly.

## Must-Haves

- `MILESTONE_ID_RE` regex exported, matches both `M001-d24bw5` and `M-abc123-001` (anchored)
