...
3. Create `src/resources/extensions/gsd/tests/integration-mixed-milestones.test.ts` with:
   ...
   - **Group 2: deriveState with mixed-format milestones** (~15 assertions): Create `M001-d24bw5/` (complete — all slices checked, has summary) + `M-abc123-002/` (active with slices). Assert: registry length 2, sorted by seq number (M001-d24bw5 first, M-abc123-002 second), M001-d24bw5 is complete, M-abc123-002 is active, `activeMilestone.id === "M-abc123-002"`, both titles correctly stripped.
   ...
   - **Group 4: inlinePriorMilestoneSummary with mixed formats** (~5 assertions): Create `M001-d24bw5/` with `M001-d24bw5-SUMMARY.md` + `M-abc123-002/` as active. Call `inlinePriorMilestoneSummary("M-abc123-002", base)`. Assert result is non-null, contains M001-d24bw5's summary content.
