...
id: M001-d24bw5
provides:
  - "MILESTONE_ID_RE, extractMilestoneSeq, parseMilestoneId, milestoneIdSort, generateMilestonePrefix, nextMilestoneId — ID primitives for both M001-d24bw5 and M-{rand6}-{seq} formats"
...
duration: "4 slices across M001-d24bw5"
...
**Collision-resistant milestone ID generation (`M-{rand6}-{seq}`) with opt-in preference, full backwards compatibility for legacy `M001-d24bw5` format, and comprehensive regex/parser hardening across all 12+ code sites.**
...
**Success criterion: Existing `M001-d24bw5`-format milestones continue to work identically**
...
- ✅ Verified: `milestoneIdSort` comparator tested in `unique-milestone-ids.test.ts` section (d). Integration tests in S04 (62 assertions) prove mixed `M001-d24bw5` + `M-abc123-001` directories work through `deriveState` and `indexWorkspace`.
